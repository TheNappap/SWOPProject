...
public class BugTrap {
	...
	private final ProjectManager projectDAO;
	private final BugReportManager bugReportDAO;
	...
	private void createUser(Element node) {
		...
		userDAO.createUser(type, first, middle, last, username);
	}
	...
	private void createProject(Element node) throws Exception {
		...
		for (int i = 0; i < roles.getLength(); i++) {
			...
			team.addMember((Developer)userDAO.getUser(role.getAttribute("user")), Role.valueOf(role.getAttribute("role")));
		}
			
		Project project = projectDAO.createProject(name, descr,creation, start, budgetEstimate, team, null);
	...
	private void createSubsystem(Element node, Project project, model.projects.System parent) {
		...
		Subsystem sub = projectDAO.createSubsystem(name, descr, project, parent, null);
	...
	private void createBugReport(Element node) throws Exception {
		...
		String descr = node.getAttribute("description");
		Date creation = (new SimpleDateFormat("dd/MM/yyyy")).parse(node.getAttribute("creationDate"));
		Subsystem sub = projectDAO.getSubsystemWithName(node.getAttribute("subsystem"));
		BugTag tag = BugTag.valueOf(node.getAttribute("tag"));
		Issuer issuer = (Issuer)userDAO.getUser(node.getAttribute("issuer"));
		
		bugReportDAO.addBugReport(title, descr, creation, sub, issuer, new ArrayList<>(), tag);
	}
